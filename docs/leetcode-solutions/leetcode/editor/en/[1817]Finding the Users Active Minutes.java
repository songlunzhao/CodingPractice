//You are given the logs for users' actions on LeetCode, and an integer k. The l
//ogs are represented by a 2D integer array logs where each logs[i] = [IDi, timei]
// indicates that the user with IDi performed an action at the minute timei. 
//
// Multiple users can perform actions simultaneously, and a single user can perf
//orm multiple actions in the same minute. 
//
// The user active minutes (UAM) for a given user is defined as the number of un
//ique minutes in which the user performed an action on LeetCode. A minute can onl
//y be counted once, even if multiple actions occur during it. 
//
...
//The user with ID=1 performed actions at minutes 2 and 3. Hence, they have a UA
//M of 2.
...
//Output: [1,1,0,0]
//Explanation:
//The user with ID=1 performed a single action at minute 1. Hence, they have a U
//AM of 1.
//The user with ID=2 performed actions at minutes 2 and 3. Hence, they have a UA
//M of 2.
