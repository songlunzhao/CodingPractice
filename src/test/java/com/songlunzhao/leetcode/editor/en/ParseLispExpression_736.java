...
//
// 
// An expression is either an integer, let expression, add expression, mult expr
//ession, or an assigned variable. Expressions always evaluate to a single integer
//. 
// (An integer could be positive or negative.) 
// A let expression takes the form "(let v1 e1 v2 e2 ... vn en expr)", where let
// is always the string "let", then there are one or more pairs of alternating var
//iables and expressions, meaning that the first variable v1 is assigned the value
// of the expression e1, the second variable v2 is assigned the value of the expre
//ssion e2, and so on sequentially; and then the value of this let expression is t
//he value of the expression expr. 
// An add expression takes the form "(add e1 e2)" where add is always the string
// "add", there are always two expressions e1, e2 and the result is the addition o
//f the evaluation of e1 and the evaluation of e2. 
// A mult expression takes the form "(mult e1 e2)" where mult is always the stri
//ng "mult", there are always two expressions e1, e2 and the result is the multipl
//ication of the evaluation of e1 and the evaluation of e2. 
...
//Additionally, for your convenience, the names "add", "let", and "mult" are prote
//cted and will never be used as variable names. 
...
//egal. Please see the examples for more details on the scope. 
// 
//
// 
// Example 1: 
...
public class ParseLispExpression_736 {
    ...
    @Test
    public void testParseLispExpression(){
       Solution solution = new ParseLispExpression_736()
                        .new Solution();
    }
