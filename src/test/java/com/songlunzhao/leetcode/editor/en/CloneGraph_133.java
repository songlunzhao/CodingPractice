...
public class CloneGraph_133 {
    ...
    public void testCloneGraph(){
       Solution solution = new CloneGraph_133()
                        .new Solution();
    }
...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        ArrayList<Node> clonedNeighbors = null;
        if(node.neighbors!=null){
            clonedNeighbors = new ArrayList<>();
        }
        Node root = new Node(node.val, clonedNeighbors);
        cloned.put(node,root);
        if(node.neighbors!=null){
            for(Node n: node.neighbors){
                clonedNeighbors.add(cloneNode(n,cloned));
            }
        }
        return root;
    }
    ...
    private Node cloneNode(Node node, HashMap<Node, Node> cloned) {
        ...
        Node clone = new Node(node.val,clonedNeighbors);
